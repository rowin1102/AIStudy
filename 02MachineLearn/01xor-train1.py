...
""" 분류기(Classifier) 객체 생성 
    SVC(Support Vector Classification) : 서포트 벡터 머신 분류 모델을 생성하는 클래스로,
    분류 작업에 적합한 SVM 모델 객체를 사용한다. 이 객체를 통해 데이터를 학습(fit)하고 
    예측(predict)을 수행할 수 있다. """
clf = svm.SVC()
clf.fit(data, label)

# 데이터 예측하기 : 예측하고 싶은 데이터를 전달하면 데이터 수만큼 예측 걀과를 리턴해준다.
