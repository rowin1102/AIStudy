...
# 필요한 열 추출
""" 첫 번쨰 열은 라벨, 두 번쨰부터는 특성 데이터이므로 추출하여 저장한다. """
...
""" SVM의 다양한 매개변수 조합을 설정할 수 있다.
    C : 정규화 강도. 설정한 강도에 따라서 얼만큼 완벽한 분리를 할 수 있을지를 결정할 수 있다.
    kernel : 커널함수의 종류. 데이터를 어떻게 분류할 것인가를 결정한다.(linear, rbf, poly 등)
    gamma : RBF 커널의 감마 값. 데이터의 패턴을 설정할 수 있다. """
params = [
