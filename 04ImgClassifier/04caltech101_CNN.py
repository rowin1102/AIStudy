...
# 입력층 : 첫 번째 합성공(Convolution) 층
""" 3*3 크기의 필터 32개 적용하고
    padding : 출력 크기를 동일하게 유지하는 same으로 설정
    input_shape : 입력 데이터 형태 지정(64*64*채널수)"""
...
""" batch_size, epochs는 딥러닝에서 사용하는 하이퍼 파라미터
    batch_size : 배치크기로 전체 데이터를 한번에 모두 처리한느 대신 작은 덩어리(batch)로
        나누어서 처리한다. 즉, 훈련에 사용하기 위한 데이터의 개수를 표현한다.
    epochs : 전체 데이터를 몇 번 반복해서 훈련할지 지정한다.
        에폭의 수가 많으면 훈련 데이터에 대해 더 많이 학습하게 되지만, 너무 많으면 과적합(Overfitting)이
        발생할 수 있으므로 적절하게 지정해야 한다. """