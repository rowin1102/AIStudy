...
""" sort() : 배열의 요소를 정렬된 상태로 반환. 오름차순 정렬이 기본.
        원본 배열은 유지한채로 정렬된 복사본을 반환한다.
    형식] 
        np.sort(배열, axis=정렬할 축, kind=정렬알고리즘) """
...
""" 슬라이싱 문법[start:stop:step]에서 step만 -1로 설정하면 배열을 역방향으로
    순회하라는 의미로 사용된다. 이 경우 start가 마지막 인덱스가 되고 stop이 
    시작인덱스가 된다. """
