...
""" ndarray를 편하게 생성 : 테스트용으로 데이터를 만들거나 대규모의 데이터를 일괄적으로
        초기화하는 경우에 사용된다. 
    arrange() : 파이썬의 표준함수인 range()와 유사하게 배열 생성
    ones() : 배열 생성 후 1로 초기화. 단, 인자로 dtype을 정해주지 않으면 float64타입으로 초기화됨.
...
""" reshape() : ndarray의 차원과 크기를 변경하는 함수. -1을 인수로 사용하면 원래
        ndarray와 호환되는 새로운 shape로 변환해줌. reshape(5, -1)이라면
        5행 x열의 배열로 변환된다. """
...
# 3차원 ndarray를 2차원 배열로 변환한다. x행 1열로 변환하므로 8행1열인 2차원 배열로 변환된다.
